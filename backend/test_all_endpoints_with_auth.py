#!/usr/bin/env python
"""Test script to check ALL backend endpoints with authentication"""
...
def test_endpoint(url, method='GET', data=None, headers=None, expected_status=None, auth_required=False, auth_token=None):
...
def get_auth_token():
    ...
    auth_url = urljoin(API_BASE, 'auth/token/')
    auth_data = {
        'username': 'admin',
        'password': 'admin12345'
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if access_token:
                ...
                print(f"Could not get access token: {token_data}")
                return None
        else:
...
def main():
    print("Testing ALL backend endpoints with authentication...")
    ...
    if not auth_token:
        print("\n⚠️  Could not get authentication token. Testing endpoints without auth (will likely fail).")
        auth_token = None
    
    # Test authentication endpoints
    print("\nTesting Authentication Endpoints:")
    auth_tests = [
        ('POST', urljoin(API_BASE, 'auth/token/'), {"username": "admin", "password": "admin12345"}, 200 if auth_token else [200, 500]),  # May fail due to Redis
        ('POST', urljoin(API_BASE, 'auth/token/refresh/'), {"refresh": "dummy_token"}, 401),  # Should return 401 for invalid token
        ('POST', urljoin(API_BASE, 'auth/token/verify/'), {"token": auth_token or "dummy_token"}, [200, 401, 500]),  # May fail due to Redis
    ]
    ...
    print("\nTesting ViewSet List Endpoints (with authentication):")
    viewset_list_tests = [
    ...
    for method, url in viewset_list_tests:
        success, status = test_endpoint(url, method, auth_required=True, auth_token=auth_token)
        all_tests.append((f"ViewSet List - {url.split('/')[-2]}", success))
    
    # Test dashboard endpoints
    print("\nTesting Dashboard Endpoints (with authentication):")
    dashboard_tests = [
    ...
    print("\nTesting Video Processing Endpoints (with authentication):")
    video_tests = [
    ...
    print("\nTesting Live Stream Endpoints (with authentication):")
    stream_tests = [
    ...
    print("\nTesting Alert Management Endpoints (with authentication):")
    alert_tests = [
    ...
    print("\nTesting Configuration Endpoints (with authentication):")
    config_tests = [
        ('GET', urljoin(API_BASE, 'config/camera/123e4567-e89b-12d3-a456-42614174000/')),  # Using dummy UUID
    ...
    print("\nTesting Camera-Specific Endpoints (with authentication):")
    ...
    print("\nTesting Rule-Specific Endpoints (with authentication):")
    rule_specific_tests = [
    ...
    print("\nTesting Event-Specific Endpoints (with authentication):")
    ...
    print("\nTesting Video File-Specific Endpoints (with authentication):")
    ...
    print("\nTesting Clip-Specific Endpoints (with authentication):")
    clip_specific_tests = [
